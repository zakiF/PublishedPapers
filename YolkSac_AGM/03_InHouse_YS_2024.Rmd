...
seGeneName_nonKit_neg <- se
```

## Creating YS only data

As we are interested in the Kit negative population, for a start lets add this population back

```{r}
# Select the KIT cnegative cells to retain
seKit_neg <- seFull[, seFull$condition %in% c("YS_E9.0_HE_Gfi1_Kneg", "YS_E9.5_HE_Gfi1_Kneg", "YS_E10_HE_Gfi1_Kneg")]


# Select the filtered cells
se_nonKit_neg <- seFull[, seFull$Barcode %in% SelctedCells$Barcode]


# Generate a new scRNA-seq object
dat1 <- assay(seKit_neg, "counts")
dat2 <- assay(se_nonKit_neg, "counts")
dat <- cbind(dat1,dat2)

c.anno1 <- colData(seKit_neg)
c.anno2 <- colData(se_nonKit_neg)
c.anno <- rbind(c.anno1, c.anno2)

g.anno <- rowData(seKit_neg)
colnames(g.anno)[5] <- "chr_name"
...
seNew$condition <- factor(seNew$condition, levels=com_popFac)


# Repeat gene conversion name and removal of non-expressing genes
row_sub <- rowSums(dat)
row_sub <- names(row_sub)[row_sub > 0]
...
new.row.names <- uniquifyFeatureNames(
    rowData(seNew)$ID,
    rowData(seNew)$Symbol
)

rownames(seNew) <- new.row.names
```
...
Subset to the selected cells, and re-do the normalizaions HVG selection, dim reduction etc

```{r}
se <- seNew # ------- IMPORTANT TO DO THIS
...
Plot the numnber of genes detected
...
df_plot <- df_cData_sub %>%
  dplyr::mutate(Estage = gsub("_Endo", "", condition),
                ...
                Cond_fac2 = gsub(".*_Endo", "Endo", condition),
                Cond_fac2 = gsub(".*HE_Gfi1_Kneg", "HE_Kneg", Cond_fac2),
                Cond_fac2 = gsub(".*_HE_Runx1|.*_HE_Gfi1", "HE_Kpos", Cond_fac2),
                Cond_fac2 = gsub(".*_EMP", "EMP", Cond_fac2),
                Cond_fac2 = gsub(".*_LMP", "LMP", Cond_fac2),
                Cond_fac2 = factor(Cond_fac2, levels = c("Endo", "HE_Kneg","HE_Kpos", "EMP", "LMP"))
                )
...
# Get n number on top of the plot
df_nNum <- 

# Plot
p2 <- 
  ggplot(df_plot, aes(x=Cond_fac2, y=detected, colour=Cond_fac2)) +
  geom_quasirandom(dodge.width=0.75) +
  geom_boxplot(aes(),fill=NA, colour="black", outlier.alpha = 0) +
  theme_bw()  +
  theme_custom +
  facet_wrap(~Estage) +
  scale_colour_manual(values=pal.YS_basic_kit) +
  ylab("Total Features") +
  #theme_custom +
...
table(df_plot$Cond_fac2)
table(df_plot$Estage, df_plot$Cond_fac2)
...
# Analysis with KIT negative cells

```{r}
outDir_inHouse6_scater_outlier <- file.path(outDir_inHouse6_scater, "YS_with_KIT")
dir.create(outDir_inHouse6_scater_outlier)
```
...
dec.se <- modelGeneVar(se)
top.se <- getTopHVGs(dec.se, n=3500)
top.se <- getTopHVGs(dec.se, prop=0.1)
chosen <- top.se
...
my.clusters <- unname(cutreeDynamic(my.tree, distM=as.matrix(my.dist), verbose=0, minClusterSize=10, deepSplit=1))
...
pdf(file.path(outDir_inHouse6_scater_outlier, "Dend_withKIT.pdf"),height = 4, width = 12)
plot(dend2)
colored_bars(cFACS, dend2, sort_by_labels_order = FALSE, y_shift=10)
...
png(file.path(outDir_inHouse6_scater_outlier, "facet_cluster_tree_refactor.png"), height=750, width = 1200)
p3.5
dev.off()
```
...
# Add a colour to show HE kit negative
df_tmp <- umap.df.int %>% 
  dplyr::mutate(Cond_fac = gsub(".*_Endo", "Endo", condition),
                Cond_fac = gsub(".*HE_Gfi1_Kneg", "HE_Kneg", Cond_fac),
                Cond_fac = gsub(".*_HE_Runx1|.*_HE_Gfi1", "HE_Kpos", Cond_fac),
                Cond_fac = gsub(".*_EMP", "EMP", Cond_fac),
                Cond_fac = gsub(".*_LMP", "LMP", Cond_fac),
                Cond_fac = factor(Cond_fac, levels = c("Endo", "HE_Kneg","HE_Kpos", "EMP", "LMP"))
                )

my.tree_or_facs <- df_tmp$Cond_fac[order.dendrogram(my.dend)]
...
cols <- pal.YS_clust_simple[c(5, 6, 1, 2, 3,4)]
...
pdf(file.path(outDir_inHouse6_scater_outlier, "Dend_reOrder.pdf"),height = 4, width = 12)
plot(dend2)
plot(UniqClus, ylim = c(-1, length(UniqClus)))
text(x =1:length(UniqClus), y=-1, UniqClus)
...
# Specific colours
pal.YS_basic_1 <- c(pal.YS_basic_kit[1], rep("#FFFFFF", 4)) # Only endo
pal.YS_basic_2 <- c(rep("#FFFFFF", 1), pal.YS_basic_kit[2], rep("#FFFFFF", 3)) # Only HE Kit Neg
pal.YS_basic_3 <- c(rep("#FFFFFF", 2), pal.YS_basic_kit[3], rep("#FFFFFF", 2)) # Only HE Kit Pos
pal.YS_basic_4 <- c(rep("#FFFFFF", 3), pal.YS_basic_kit[4], rep("#FFFFFF", 2)) # Only EMP
pal.YS_basic_5 <- c(rep("#FFFFFF", 4), pal.YS_basic_kit[5]) # Only LMP
...
c.FACS_5 <- pal.YS_basic_5[df_order$FACS] 
...
colored_bars(c.FACS_5, dend2, sort_by_labels_order = FALSE, y_shift=-500)
...
What percentage of a cell population is in each cluster

```{r}
df_popPerc <- df_tmp %>% 
  group_by(Cond_fac, TreeCut_re) %>% 
...
ann_list <- list(Cluster = 
                   c("k1" = pal.YS_clust_simple[1],
                     ...
                     "k6" = pal.YS_clust_simple[6]),
                 FACS = 
                   c("Endo" = pal.YS_basic_kit[1],
                     "HE_Kneg" = pal.YS_basic_kit[2],
                     "HE_Kpos" = pal.YS_basic_kit[3],
                     "EMP" = pal.YS_basic_kit[4],
                     "LMP" = pal.YS_basic_kit[5]),
                 Stage = 
                   c("E9.0" = pal.Estage[1],
                     "E9.5" = pal.Estage[2],
                     "E10.5" = pal.Estage[3]),
...
# -------- Plot heatmap ------------- #
pdf(file.path(outDir_inHouse6_scater_outlier, "Heatmap_CustomGenes.pdf"), width = 11, height = 7)
pheatmap(mat22,
         cluster_rows = FALSE, 
         cluster_cols = FALSE,
...
We make a violin plot of key genes expressed in each cluster
...
# 45w 77h
ggsave(filename = file.path(outDir_inHouse6_scater_outlier, "SelectedGenes.pdf"), device = "pdf", width = 6, height = 8, dpi = 300)
```
...
```{r}
saveRDS(se, file.path(outData_dir, "se_YS_only_fullInfo_2024.RDS"))
```
...
# Analysis withOUT KIT negative cells
